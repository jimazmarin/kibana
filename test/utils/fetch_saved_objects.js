...
export const finalLocations = (dest) => (filePath) => {
  const destDir = join(REPO_ROOT, dest);
  const destFilePath = join(destDir, filePath);
  return [destDir, destFilePath];
};

export const fetchSavedObjects = (dest) => (log) => async (supertest) => {
  const [destDir, destFilePath] = finalLocations(dest)('./exported.ndjson');
